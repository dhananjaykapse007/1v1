...
exports.buchholz = function(contest_id, round){         // 1... round inlcusive     (round is 1-based)
        ...
        .then(async doc=>{
            ...
            for(let [player, stat] of doc.results[round-1].table){
                ...
                for(let k=0; k<opponents.length; k++){          // only for this round
                    oppo_score+= doc.results[round-1].table.get(opponents[k].opp).score;
                }
    
                console.log("BH for", player, ":", oppo_score) 
                doc.results[round-1].table.get(player).buchholz = oppo_score;             // updating the doc
            }
